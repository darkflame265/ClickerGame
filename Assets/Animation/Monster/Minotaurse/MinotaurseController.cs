...
public class MinotaurseController : MonoBehaviour
{
    Animator animator;
    GameObject player;
    ...
    public void Move()
    {
        if(movebool == true)
        {
            ...
            {
                //아래로 이동
                this.transform.position = new Vector3(transform.position.x, this.transform.position.y-1.2f, transform.position.z);
                postion_y = false;
            }
        ...
        foreach(GameObject target in enemies) {
            ...
            if(Vector3.Distance(close_enemy.transform.position, this.transform.position) < 2f) //this.transform.position
            {
                movebool = false;
                allAnimatorStop();
                animator.SetBool("isAttack", true);
